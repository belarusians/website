...
function generateSiteMap(newsSlugs: string[], eventsSlugs: string[]) {
  const newsUrls = newsSlugs.map((slug) => generateTranslatedUrls(`/news/${slug}`)).join("");
  const eventsUrls = eventsSlugs.map((slug) => generateTranslatedUrls(`/events/${slug}`)).join("");
...
export async function getServerSideProps({ res, locale }: GetServerSidePropsContext) {
  ...
  const events = getEventsSlugs(locale as Lang);

  const sitemap = generateSiteMap(news, events);
